...
describe("storybook/dev-server/pipe-with-prefix", () => {
    ...
    it("should call logger.error when pipeline callback is called with an error", () => {
        mockedPipeline.mockImplementationOnce((...args: unknown[]): NodeJS.WriteStream => {
            const callback: (error: Error) => void = args[args.length - 1] as (err: Error) => void;
            callback(new Error("Pipeline error"));

            return args[args.length - 2] as NodeJS.WriteStream;
        });
