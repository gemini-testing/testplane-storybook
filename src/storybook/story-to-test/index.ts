...
import type { Test } from "hermione";
...
const hermioneTestNameSuffix = ".hermione.js";
...
export const getStoryFile = (test: Test): string => {
    ...
    if (!testFile.startsWith(testsTmpDir) || !testFile.endsWith(hermioneTestNameSuffix)) {
        throw new Error(
            `Can't get storyfile of ${test.fullTitle()}, because it was not autogenerated by storybook-hermione`,
        );
    }

    return path.relative(testsTmpDir, testFile).slice(0, -hermioneTestNameSuffix.length);
};

const getStoryTestPath = (testsDir: string, story: StorybookStoryExtended): string =>
    path.resolve(testsDir, story.importPath + hermioneTestNameSuffix);

export const buildStoryTestFiles = async (stories: StorybookStoryExtended[], opts: HermioneOpts): Promise<string[]> => {
